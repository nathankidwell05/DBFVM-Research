...
mainLimiter = "hybridmc";  % hybridmc uses MC in smooth regions but switches toward minmod near steep/suspicious slopes
...
while time < tEnd                              % continue until final time is reached
    ...
    while ~stepAccepted                        % keep trying until the RK3 step is physically acceptable
        ...
        if retryCount < 4                                            % first few tries use the sharp limiter
            activeLimiter = mainLimiter;                             % use the hybrid MC/minmod limiter during normal operation
    ...
    if makeLivePlot && (mod(step,plotEvery) == 0 || time >= tEnd) % only update plot occasionally to save speed
        ...
        sgtitle(sprintf('KT-D1V5 + Hybrid MUSCL MC/Minmod + SSP-RK3: t = %.4f, step = %d',time,step)); % update title
...
save('d1v5_sodshock_muscl_MC_RK3_latest.mat', ...
    'x','rho','u','p','T','exact','Nx','dx','dtBase','tEnd','step','tau','CFL','L1_rho','L1_u','L1_p','L1_T','retryCountTotal','fallbackCountTotal','repairCountTotal','TFloor','rhoFloor','mainLimiter'); % save latest result for post-processing
...
sgtitle('KT-D1V5 FVDBM with Hybrid MUSCL MC/Minmod and SSP-RK3'); % final plot title
...
% MC is sharp, minmod is safer, and firstorder turns off the MUSCL slope.
    ...
    for q = 1:Q                                        % loop through each discrete velocity population
        ...
        if limiterType == "firstorder"                 % first-order upwind uses no MUSCL slope
            ...
            elseif limiterType == "hybridmc"           % use MC in smooth cells and minmod close to steep changes
                sigma(2:Nx-1) = hybrid_MC_minmod_limiter_vector(dL,dR); % switch locally before an entire RK stage fails
...
function s = hybrid_MC_minmod_limiter_vector(dL,dR)
%HYBRID_MC_MINMOD_LIMITER_VECTOR chooses MC or minmod separately in each cell.
% MC keeps smooth portions of the solution sharper and less diffusive.
% Minmod is selected near sudden or strongly one-sided changes to reduce overshoot.

    slopeMC = MC_limiter_vector(dL,dR);                % calculate the sharper MC slope in every interior cell
    slopeMinmod = minmod_limiter_vector(dL,dR);        % calculate the safer minmod slope in every interior cell

    smallNumber = 1.0e-14;                             % prevents division by zero when both changes are almost zero
    curvatureRatio = abs(dR-dL) ./ ...
        (abs(dR)+abs(dL)+smallNumber);                 % small in smooth regions and large when the slope changes suddenly
    slopeSizeRatio = max(abs(dL),abs(dR)) ./ ...
        (min(abs(dL),abs(dR))+smallNumber);            % large when one side of the cell changes much more than the other

    steepChange = curvatureRatio > 0.35;               % mark a cell when its left and right slopes differ strongly
    oneSidedChange = slopeSizeRatio > 3.0;              % mark a cell when the jump is mainly on only one side
    troubledCell = steepChange | oneSidedChange;        % either warning makes this cell use minmod

    s = slopeMC;                                       % use MC by default so smooth solution features remain sharp
    s(troubledCell) = slopeMinmod(troubledCell);        % replace MC only in cells identified as locally troubled

end                                                    % end hybrid MC/minmod limiter function
