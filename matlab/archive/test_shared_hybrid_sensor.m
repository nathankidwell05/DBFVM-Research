...
tEnd = 0.002;          % reduced final time used only for this automated smoke test
...
%% Domain
Nx = 500;                % reduced grid used only for this automated smoke test
...
%   2. collision: (feq_q - f_q)/tau
    ...
    troubledCell = build_macroscopic_troubled_cell_mask(rho,p,T); % locate shocks/contacts once so every population makes the same limiter choice
    fluxDiv = build_MUSCL_flux_divergence(f,c,dx,limiterType,troubledCell); % finite-volume transport divergence using selected limiter
    rhs = -fluxDiv + collision;                        % combine negative transport divergence with collision source

end                                                    % end RHS function

function fluxDiv = build_MUSCL_flux_divergence(f,c,dx,limiterType,troubledCell)
